  ...
  module CkFu
    def ck_fu(options={})
      ...
      content_tag :div, :id => 'ck_fu', :class => RAILS_ENV do
        ...
        text += " #{separator} Revision: #{deployed_revision}" if options[:revision] && !deployed_revision.blank?
        text += " #{separator} Deployed: #{deployed_date}" if options[:date] && !deployed_date.blank?
